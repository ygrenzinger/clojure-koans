...
(defn display-full-name [[first-name last-name]]
  (str first-name " " last-name))

(defn display-address [address]
  (let [{:keys [street-address city state]} test-address]
       (apply str (interpose ", " [street-address city state]))))

(defn display-person [full-name address]
  (str (display-full-name full-name) ", " (display-address address)))
...
(meditations
  ...
  (= "Test Testerson, 123 Test Lane, Testerville, TX"
     (display-person ["Test" "Testerson"] test-address)))
